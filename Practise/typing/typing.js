document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateUL() {
        let before = txt.substring(0, currentIndex);
        let current = txt[currentIndex];
        let after = txt.substring(currentIndex + 1);
    ...
    document.getElementById("start").addEventListener("click", () => {
        txt = generatepara();
        if(m===0){
            alert("To start, please select the duration first.");
            return;
        }
        v.innerHTML = txt;
        ...
        countdown = setInterval(function() {
            ...
            if (time < 0) {
                ...
                setTimeout(() => {
                    ...
                    document.getElementById("sp2").innerHTML = `Your WPM is ` + wpm.toFixed(2);
                }, 0);
            }
        }, 1000);
    }
